...
def test_get_health_status_healthy():
    ...
    health.update_health_state(
        prelude_available=True, prebetter_available=True, ready=True
    )
    status = health.get_health_status()
...
def test_get_health_status_degraded():
    ...
    health.update_health_state(
        prelude_available=True, prebetter_available=False, ready=True
    )
    status = health.get_health_status()
...
def test_get_health_status_unhealthy():
    ...
    health.update_health_state(
        prelude_available=False, prebetter_available=True, ready=True
    )
    status = health.get_health_status()
    assert status.status == "unhealthy"
    assert status.prelude_db is False
    assert (
        status.prebetter_db is True
    )  # Prebetter state doesn't matter if prelude is down
...
def test_get_health_status_uptime():
    ...
    assert later_status.uptime_seconds - initial_status.uptime_seconds == pytest.approx(
        sleep_time, abs=0.05
    )
...
@patch(
    "app.services.health.logger"
)  # Mock logger to suppress error messages during test
...
def test_check_database_health_invalid_db_type():
    ...
    assert result == {
        "connected": True
    }  # Still connects, just doesn't update specific state
