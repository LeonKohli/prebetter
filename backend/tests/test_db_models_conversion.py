...
def test_alert_result_to_list_item_minimal():
    ...
    assert result.analyzer.node is None # Node info depends on host, location, or category
...
def test_grouped_alert_to_response():
    ...
    result = grouped_alert_to_response(pair_row, alerts_map)
...
def test_process_grouped_alerts_details_basic():
    ...
    alerts = [
        MockRow(**alert_data_1),
        MockRow(**alert_data_2),
        MockRow(**alert_data_3)
    ]
...
def test_clean_byte_string_malformed():
    assert clean_byte_string("b'unclosed string") == "b'unclosed string" # Return original if malformed
    assert clean_byte_string("'missing b'") == "'missing b'"

def test_clean_byte_string_empty_none():
    assert clean_byte_string("") == ""
    assert clean_byte_string(None) is None
...
def test_process_additional_data_basic():
    add_data_rows = [
        ...
        MockRow(meaning="InvalidBool", type="boolean", data="maybe"), # Invalid conversion
...
def test_format_relative_time():
    ...
    assert format_relative_time(now - timedelta(minutes=1, seconds=30), now) == "1 minute ago"
    ...
    assert format_relative_time(now - timedelta(hours=1, minutes=30), now) == "1 hour ago"
...
def test_determine_heartbeat_status():
    ...
    assert determine_heartbeat_status(inactive_time, now, interval_seconds) == "inactive"
    ...
    assert determine_heartbeat_status(exact_interval_time, now, interval_seconds) == "active"
    ...
    assert determine_heartbeat_status(exact_2x_interval_time, now, interval_seconds) == "inactive"
...
def test_determine_heartbeat_status_none():
    ...
    assert determine_heartbeat_status(None, now) == "unknown" # Status is unknown if no last heartbeat 