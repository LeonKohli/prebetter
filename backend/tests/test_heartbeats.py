...
def test_heartbeats_timeline(auth_client):
    ...
    try:
        ...
        if data["items"]:
            ...
            try:
                timestamp = ensure_timezone(datetime.fromisoformat(item["time"].replace('Z', '+00:00')))
...
def test_heartbeats_timeline_pagination(auth_client):
    ...
    try:
        ...
        if data["pagination"]["pages"] > 1:
            page2_response = auth_client.get("/api/v1/heartbeats/timeline?page=2&page_size=50")
        ...
        invalid_size_response = auth_client.get("/api/v1/heartbeats/timeline?page_size=0")
        assert invalid_size_response.status_code in [400, 422]
        
        too_large_size_response = auth_client.get("/api/v1/heartbeats/timeline?page_size=1001")
...
def test_heartbeats_authentication(client):
    ...
    endpoints = [
        "/api/v1/heartbeats/status", 
        "/api/v1/heartbeats/timeline"
    ]
