...
def test_timeline_time_frames(auth_client):
    ...
    for time_frame in time_frames:
        response = auth_client.get(
            f"/api/v1/statistics/timeline?time_frame={time_frame}"
        )
        assert response.status_code == 200
        data = response.json()
...
def test_timeline_group_by(auth_client):
    ...
    for group_by in group_by_options:
        response = auth_client.get(
            f"/api/v1/statistics/timeline?time_frame=hour&group_by={group_by}"
        )
        assert response.status_code == 200
        data = response.json()
    ...
    response = auth_client.get(
        "/api/v1/statistics/timeline?time_frame=hour&group_by=invalid"
    )
    assert response.status_code == 200
    data = response.json()
